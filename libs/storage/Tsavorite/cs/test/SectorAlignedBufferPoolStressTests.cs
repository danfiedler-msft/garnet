...
namespace Tsavorite.test
{
    ...
    /// Reproduces the real IO asymmetry (issuing threads <c>Get</c>, arbitrary completion threads <c>Return</c>
    ...
    /// (allocations vs working set); A/Bs origin-return vs the legacy single-queue vs native mimalloc.
    ...
    public unsafe class SectorAlignedBufferPoolStressTests
    {
        ...
        // ========================================================================================================
        // 1. Scaling throughput A/B (magazine fast path): tight per-thread Get/Return loop across a thread sweep,
        //    origin-return vs legacy single-queue vs native mimalloc. Reproduces the RootCause table and proves
        //    the per-thread magazine design scales instead of collapsing on a shared queue's cache-line ping-pong.
        // ========================================================================================================

        [Test]
        public void ScalingThroughputAB()
        {
            var haveMimalloc = Mimalloc.TryInitialize();
            const int bufferSize = 4096;          // typical record-sized IO buffer (pooled, magazine fast path)

            TestContext.Progress.WriteLine("threads | origin-return | legacy-1queue | mimalloc | or/legacy");
            TestContext.Progress.WriteLine("--------+---------------+---------------+----------+----------");
            foreach (var threads in new[] { 1, 2, 4, 8, 16, 32, 64 })
            {
                // Full op budget for the scalable paths; cap the legacy single-queue at high thread counts where it
                // collapses to <1 Mops/s (the pathology this A/B demonstrates) so the sweep completes in bounded time.
                var scalableOps = 5_000_000L;
                var legacyOps = threads >= 16 ? 1_000_000L : 5_000_000L;

                var origin = RunTightLoop(useOriginReturn: true, native: false, threads, bufferSize, scalableOps);
                var legacy = RunTightLoop(useOriginReturn: false, native: false, threads, bufferSize, legacyOps);
                var mim = haveMimalloc ? RunTightLoop(useOriginReturn: true, native: true, threads, bufferSize, scalableOps) : 0;

                TestContext.Progress.WriteLine(
                    $"{threads,7} | {origin / 1e6,13:F1} | {legacy / 1e6,13:F1} | {mim / 1e6,8:F1} | {(legacy > 0 ? origin / legacy : 0),8:F1}x");
            }
        }

        static double RunTightLoop(bool useOriginReturn, bool native, int threads, int bufferSize, long opsPerThread)
        {
            SectorAlignedBufferPool.UseOriginReturn = useOriginReturn;
            SectorAlignedBufferPool.NativeAllocator = native ? new MimallocPooledAllocator() : null;
